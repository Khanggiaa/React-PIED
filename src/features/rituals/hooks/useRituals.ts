...
export function useRituals() {
  ...
  const filters = useMemo<RitualFilterParams>(() => {
    //useMemo để tránh parse lại filters mỗi lần component re-render nếu searchParams không thay đổi
    return {
      page: Number(searchParams.get("page")) || 1,
      limit: Number(searchParams.get("limit")) || 10,
      search: searchParams.get("search") || undefined,
      difficultyLevel: searchParams.get("difficultyLevel") || undefined, //undefined để tránh gửi param rỗng, chỉ gửi khi có giá trị
      ritualCategoryId: searchParams.get("ritualCategoryId") || undefined,
    };
  }, [searchParams]); //dependencies là searchParams, chỉ parse lại khi searchParams thay đổi

  const query = useQuery({
    queryKey: [...QUERY_KEYS.RITUALS, filters],
    queryFn: () => ritualService.getAll(filters),
    placeholderData: (prev) => prev, // hiện data cũ trong khi loading mới
  });
